  import ikrZoom from './ikr-zoom.js';
  import { edgeToLineMap } from './edgeToLineMap.js';
        ...
        function findAndHighlightRoute(start, end) {
          ...
          for (let i = 0; i < path.length - 1; i++) {
            for (let j = i + 1; j < path.length; j++) {
              ...
              if (edge && !highlightedEdges.has(edge.id)) {
                ...
                edge.element// keep base class: do not remove 'map_road'
                ...
                if (lineElement) {
                  lineElement// keep base class: do not remove 'map_road'
                  // Apply highlight_path for path and polygon, highlight for line
          ...
          path.forEach((node, index) => {
            ...
            if (vertex && !node.startsWith('road_')) {
              ...
              if (line) {
                if (index === 0) {
                  line// keep base class: do not remove 'map_road'
